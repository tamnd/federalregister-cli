...
describes what users get. Pick a format with `-o`, or let fedreg choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
fedreg <command> -o table   # aligned columns for reading
fedreg <command> -o jsonl   # one JSON object per line, for piping
fedreg <command> -o json    # a single JSON array
fedreg <command> -o csv     # spreadsheet friendly
fedreg <command> -o tsv     # tab-separated
fedreg <command> -o url     # just the URL column
fedreg <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
fedreg <command> --fields id,title,url
```
...
```bash
fedreg <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
fedreg <command>            # a table, because this is a terminal
fedreg <command> | wc -l    # JSONL, because this is a pipe
```
